...
- Parser tests cover WWHAA section detection and IVQ question/option extraction
...
Requires `ANTHROPIC_API_KEY` in `.env` file (see `.env.example`). Default model is `claude-sonnet-4-20250514` (configurable via `MODEL` env var). Max tokens: 4096. Default duration: 7 minutes at 150 WPM. TTS voice configurable via `TTS_VOICE` env var (default: `en-US-AriaNeural`).
